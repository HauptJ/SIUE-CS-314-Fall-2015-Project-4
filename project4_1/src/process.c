...
//------------------------------------------
  ...
  //clear each entry 1 at a time in the page table
  //printf("Heir ist MemoryFreePage() clear\n");
  for (i = 0; i < L1_MAX_ENTRIES; i++) {
    //printf("MemoryFreePage() clear entered for loop\n");
    if(pcb->pagetable[i] != 0) {
      //debug output
      //printf("MemoryFreePage() clear entered if\n");
      //printf("Heir ist MemoryFreePage() clear\n");
      //printf("clearing page table\n");
      //printf("PID :%lu\n", findpid(pcb));
      //printf("Physical Page Number :\n",&pcb->pagetable[i],i);
      //debug output
      MemoryFreePte (pcb->pagetable[i]);
    }
  }
...
{
  int		i, j, fd, n; //TY for the counter variables :)
  Link		*l;
...
//Given a page number, return the page table entry.
 ...
 //comment me
  ...
  //Allocate 3 pages of test & data
  //i is set to 3 for 3 pages to tests and data as per PPT
  for (i = 0; i < 3; i++){
    newPage = MemoryAllocPage ();
    if (newPage == 0) {
      printf ("aFATAL: couldn't allocate memory [3 pages of test & data] - no free pages!\n");
      exitsim ();	// NEVER RETURNS!
    }
    //pcb->pagetable[0] = MemorySetupPte (newPage); //original
    pcb->pagetable[i] = MemorySetupPte (newPage);
  }

  //Allocate 1 page of user stack when creating a process
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
    printf ("aFATAL: couldn't allocate memory [1 page of user stack] - no free pages!\n");
    exitsim ();	// NEVER RETURNS!
  }
  pcb->pagetable[L1_MAX_ENTRIES-1] = MemorySetupPte (newPage);
  

  //system stack 1 page for it
  //stays same
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
...
//----------------------------------------------
  ...
  //stackframe[PROCESS_STACK_PTSIZE] = pcb->npages; //original
...
{
  // add your code below - NEIN
  //I don't want to though :(

  //printf("ProcessKill() ist heir!\n"); //debug
  ...
  //destroy process 
  ProcessDestroy (pcb); //destroy process to prevent infinite loop
  ProcessSchedule ();
}
...
{
	// add your code below - NO 
  uint32 *tempstackframe;
  uint32 faultAddress; //page fault address

  int newPage; //Allocate new page
  int page; //Fix page table


  //printf ("Das PageFaulthandler ist hier!\n"); //debug
	
  tempstackframe = currentPCB->currentSavedFrame ;
  faultAddress = tempstackframe[PROCESS_STACK_FAULT];


  //check PCB for number of allocated pages


  //if allocated pages > 16 virtual pages
  //If there are more than 64 allocated virtual memory pages, we have ran out of memory. Therefere we killed the process.
  //Error referenced in userprog2.c
  //Wir haben ein Error! Nicht gut!
  if (currentPCB->npages == 16){ //64 pages per processes [8kb * 16 pages * 4 bytes per addressable space = 512 kb]
  //below if is the equivalent of the one above
  //if (currentPCB->npages * MEMORY_PAGE_SIZE > 524288){ //[1 << 19] = 524288
    printf("Ach Nein! Kein Mehr Frei Memory :( \n"); //debug
    printf("NO! No more free memory :( \n"); 
  ...
  newPage = MemoryAllocPage();
  if(newPage == 0) {
    printf("Ach nein! Neue Seite falsch zugeordnet\n");
    printf("NO! Memory Allocated wrong\n");
    return ProcessKill(currentPCB);
    //exitsim();
  }
  ...
  //Print!
  //printf("Die Messwerte aus PageFaultHandler() \n");
  //printf("The results from PageFaultHandler() \n");
  /*printf("Process id %lu\n", findpid(currentPCB));
  printf("page fault addr %p\n", faultAddress);
  printf("Phys page number alloc: %d\n", page);*/

  //printf report the dynamic page allocation
  //printf("PID:%2d\tPage Fault Address:%8x\tPhysical Page:%d|%d\n",currentPCB-pcbs,faultAddress,page); 

   printf("PID:%2d\tPage Fault Address:0x%8x\tPhysical Page:%d >> %d\n",currentPCB-pcbs,faultAddress,newPage,page); 
}
