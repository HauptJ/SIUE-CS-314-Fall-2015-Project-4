...
//-------------------------------------------
    ...
    uint32 *L2pt;
    int L1p = (addr / MEMORY_PAGE_SIZE) / L2_MAX_ENTRIES;
    int L2p = (addr / MEMORY_PAGE_SIZE) % L2_MAX_ENTRIES;
    
    int offset = addr % MEMORY_PAGE_SIZE;

   if((L1p*L2p) > ((L1_MAX_ENTRIES-1)*(L2_MAX_ENTRIES-1))) {
      printf(":(\n");
      return NULL;
    }

    L2pt = (uint32 *)(pcb->pagetable[L1p] & MEMORY_PTE_MASK);//strip our PTE bits they nest within the actual address
    L2pt += L2p;//the l2 table offset within the l1 table
    return ((*L2pt & MEMORY_PTE_MASK)+ offset); //the location in physical 
}
