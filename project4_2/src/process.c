...
//------------------------------------------------------------
  ...
  //Allocate a page for L2 page table
  newPage = MemoryAllocPage ();
  if (newPage == 0) { //Check to see if allocation is valid
    printf ("aFATAL: couldn't allocate memory - no free pages!\n");
    exitsim ();	// NEVER RETURNS!
  }
  pcb->pagetable[0] = MemorySetupPte (newPage);//Setup L1 entry for L2 page table using MemmorySetupPte()
  L2PageTable = (uint32 *)(pcb->pagetable[0] & MEMORY_PTE_MASK);// Calculate L2 page address: L2 = L1[L1_page_no] & (MEMORY_PAGE_MASK)
  
  //Allocate pages for actual text, data, or stack
  for(i=0;i<3;i++) {
    newPage = MemoryAllocPage();
    if (newPage == 0){//check if allocation fails
      exitsim();
    } else {
      *(L2PageTable+i) = MemorySetupPte(newPage); 
    }
  }
  
  //Set up last L2PageTable entry
    newPage = MemoryAllocPage();
    if (newPage == 0) {
      printf ("bFATALnewPage = MemoryAllocPage () : couldn't allocate system stack - no free pages!\n");
      exitsim ();	// NEVER RETURNS!
    }
    L2PageTable = (uint32*)(pcb->pagetable[L1_MAX_ENTRIES - 1] & MEMORY_PTE_MASK);
    
    //Set up 1 page for user stack in last L2Page of the L2PageTable
  if (newPage == 0) {
    printf ("bFATALnewPage = MemoryAllocPage () : couldn't allocate system stack - no free pages!\n");
    exitsim ();       // NEVER RETURNS!
  }
  *(L2PageTable + L2_MAX_ENTRIES - 1) = MemorySetupPte(newPage);

  //Set up 1 for system stack
  newPage = MemoryAllocPage();
  if (newPage == 0) {
    printf ("bFATALnewPage = MemoryAllocPage () : couldn't allocate system stack - no free pages!\n");
    exitsim ();       // NEVER RETURNS!
...
void PageFaultHandler()
{
  ...
  int newPage, L1page, L2page;
  uint32 *L2PageTable;
  //int L1Page, L2Page;
  tempstackframe = currentPCB->currentSavedFrame ;
  fault_address = tempstackframe[PROCESS_STACK_FAULT];

  L1page = (fault_address/MEMORY_PAGE_SIZE)/L2_MAX_ENTRIES;
  L2page =(fault_address/MEMORY_PAGE_SIZE)%L2_MAX_ENTRIES;

  //CHECK TO SEE IF WE ARE TOTALLY OUT OF MEMORY NOW --> KILL PROCESS

  newPage = MemoryAllocPage();
  if (newPage == 0) {//this is the fail safe- but shouldnt happen in our controlled scenario
    printf ("No free pages remaining to allocate![system]PageFaultHandler()\n");
    exitsim ();       // NEVER RETURNS!
  }

  //Create a L2PageTable
  if(currentPCB->pagetable[L1page]==0){//If no Page in L1, make a page to hold L2PageTable
    currentPCB->pagetable[L1page]=MemorySetupPte(newPage);
  }
  L2PageTable = (uint32*)(currentPCB->pagetable[L1page] & MEMORY_PTE_MASK);//Make L2PageTable

  //create a page to put inside L2
  newPage = MemoryAllocPage();
  if (newPage == 0) {
    printf("No page available to assign to L2\n"); //printf with else
    exitsim ();       // NEVER RETURNS!
  }
  *(L2PageTable+=L2page)=MemorySetupPte(newPage);

  //increment npages
  currentPCB->npages = (currentPCB->npages)+1;

  //printf report the dynamic page allocation
  printf("PID:%2d\tPage Fault Address:0x%8x\tPhysical Page:%d >> %d|%d\n",currentPCB-pcbs,fault_address,newPage,L1page,L2page); 
}